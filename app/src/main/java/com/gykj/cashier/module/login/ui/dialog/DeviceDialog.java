package com.gykj.cashier.module.login.ui.dialog;
...
import com.gykj.cashier.module.login.ui.adapter.DeviceAdapter;
...
public class DeviceDialog extends Dialog implements BaseQuickAdapter.OnItemChildClickListener{
    ...
    public void OnClisk(View view){
        switch (view.getId()){
            ...
            case R.id.login_certain_tv:
                DeviceEntity deviceListBean = getDeviceListBean();
        ...
        switch (view.getId()){
            case R.id.tab_layout:
                for(int i = 0;i<mDeviceList.size();i++){
                    if(position == i){
                        mDeviceList.get(i).setCheck(true);
                    }else {
                        mDeviceList.get(i).setCheck(false);
                    }
                }
                adapter.setNewData(mDeviceList);
                break;
    ...
    private DeviceEntity getDeviceListBean(){
        DeviceEntity deviceListBean = null;
        for(int i = 0;i<mDeviceList.size();i++){
            if(mDeviceList.get(i).isCheck()){
